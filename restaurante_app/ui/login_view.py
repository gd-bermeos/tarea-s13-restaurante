...
class LoginView(ttk.Frame):
    ...
    def _construir_interfaz(self):
        ...
        titulo = ttk.Label(self, text="Restaurante App", font=("Arial", 22, "bold"))
        titulo.grid(row=0, column=0, columnspan=2, pady=(10, 5))

        subtitulo = ttk.Label(self, text="Inicio de sesión", font=("Arial", 12))
        ...
        entrada_usuario = ttk.Entry(self, textvariable=self.usuario_var, width=28)
        ...
        entrada_contrasena = ttk.Entry(self, textvariable=self.contrasena_var, show="*", width=28)
        entrada_contrasena.grid(row=3, column=1, sticky="w", padx=8, pady=8)

        ttk.Button(self, text="Ingresar", command=self._intentar_ingreso).grid(
            row=4, column=0, columnspan=2, pady=14
        )

        mensaje = ttk.Label(self, textvariable=self.mensaje_var, font=("Arial", 10))
        mensaje.grid(row=5, column=0, columnspan=2, pady=5)

        ayuda = ttk.Label(self, text="Credencial de prueba: admin / admin123", font=("Arial", 9))
        ayuda.grid(row=6, column=0, columnspan=2, pady=(15, 0))
